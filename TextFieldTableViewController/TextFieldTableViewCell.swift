...
open class TextFieldTableViewCell: UITableViewCell {
  ...
  public override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
    ...
    textFieldLeftLayoutConstraint = NSLayoutConstraint(
      item: textField,
      attribute: .leading,
      relatedBy: .equal,
      toItem: contentView,
      attribute: .leading,
      multiplier: 1,
      constant: separatorInset.left+1
    )
...
extension UITableView {
  public func indexPath(for textField: UITextField) -> IndexPath? {
    indexPath(for: textField.superview!.superview! as! UITableViewCell)
  }

  public func textFieldForRow(at indexPath: IndexPath) -> UITextField? {
    (cellForRow(at: indexPath) as! TextFieldTableViewCell?)?.textField
  }
}
